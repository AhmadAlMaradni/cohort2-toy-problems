...
var dirReduc = function(directions){
    for (var i = 0; i < directions.length; i++) {
        if(directions[i]+directions[i+1] !== "NORTHSOUTH"){
            directions.shift()
          if(directions[i]+directions[i+1] !== "SOUTHNORTH"){
            directions.shift()
            if(directions[i]+directions[i+1] !== "EASTWEST"){
                directions.shift()
                if (directions[i]+directions[i+1] !== "WESTEAST"){
                    directions.shift()
                }
            }
          }
